...
import subprocess
import glob
# Find the exact entry script for each supplementary-figure directory.
figure_dirs = glob.glob('supp_figures/SFig*')
fig_scripts = [
...
# Sort by manuscript supplementary-figure number.
fig_scripts.sort(key=lambda path: int(os.path.basename(os.path.dirname(path))[4:]))
