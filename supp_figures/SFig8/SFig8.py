...
def main() -> None:
    ...
    if simspace_version != SIMSPACE_EXPECTED_VERSION:
        raise RuntimeError(
            "Supplementary Figure 8 requires SimSpace "
    ...
    if len(frozen_simulated_metadata) != len(simulated_counts):
        raise ValueError("Figure 3 simulated metadata/count rows do not align.")
    ...
    for seed in SIMULATION_SEEDS:
        ...
        if seed == 0:
            ...
            if not (same_state and same_coordinates):
                raise ValueError(
                    "Seed-0 replay does not match the frozen Figure 3 metadata."
                )
    ...
    local_output = SCRIPT_DIR / "SFig8.png"
    example_output = EXAMPLE_DIR / "SFig8.png"
