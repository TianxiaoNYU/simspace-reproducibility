"""Supplementary Figure 10: quantitative reference-free 3D simulation.
...
renders from archived results; SFig10_generate_data.py triggers the full
...
EXAMPLE_DIR = REPO_ROOT / "example_output" / "SFig10"
...
def save_figure(
    ...
    figure.savefig(SCRIPT_DIR / "SFig10.png", dpi=300, bbox_inches="tight")
    figure.savefig(EXAMPLE_DIR / "SFig10.png", dpi=300, bbox_inches="tight")
    plt.close(figure)
...
def load_archived_results() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if missing:
        raise FileNotFoundError(
            "Archived SFig10 results are missing. Run "
            "SFig10_generate_data.py first. Missing: "
            + ", ".join(missing)
        )
...
def main() -> None:
    ...
    print(
        "Rendered SFig10 from archived ten-seed results. "
        "Run SFig10_generate_data.py to regenerate the simulations."
    )
