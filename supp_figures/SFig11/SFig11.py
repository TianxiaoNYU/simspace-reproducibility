...
for i, plt_kernel in enumerate(plt_kernel_size):
    ...
    plt.tight_layout()
    plt.savefig(f'{script_dir}/Fig5_panel_A{i+1}.png', dpi=300)
    ...
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title ='Metric')
    plt.tight_layout()
    plt.savefig(f'{script_dir}/Fig5_panel_B{i+1}.png', dpi=300)
    plt.figure(figsize=(6.5, 5), dpi=300)
    sns.boxplot(
    ...
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title ='Metric')
    plt.tight_layout()
    plt.savefig(f'{script_dir}/Fig5_panel_C{i+1}.png', dpi=300)