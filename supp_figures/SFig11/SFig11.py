...
The analysis compares the fixed reference-free Figure 2 cortex simulation
...
EXAMPLE_DIR = REPO_ROOT / "example_output" / "SFig11"
RAW_ARCHIVE = DATA_DIR / "STARmap_mouse_visual_cortex.zip"
FIG2_DIR = REPO_ROOT / "main_figures" / "Fig2_new"
SIMULATED_META = FIG2_DIR / "Panel_A_C_data" / "anchor_cells.tsv.gz"
SIMULATED_EXPRESSION = FIG2_DIR / "Panel_C_data" / "simulated_expression.tsv.gz"
...
def verify_inputs() -> None:
    ...
    if missing:
        raise FileNotFoundError(
            "Missing inputs: "
            + ", ".join(missing)
            + ". Run download_starmap.py and Fig2_new.py as needed."
        )
    observed = {
...
def load_simulated() -> pd.DataFrame:
    ...
    if len(cells) != len(expression):
        raise ValueError("Figure 2 metadata and expression rows are misaligned.")
    ...
    if cells[["cell_class", "spatial_domain"]].isna().any().any():
        raise ValueError("An unmapped Figure 2 cell type or niche was found.")
...
def write_outputs(
    ...
    configuration = {
        "status": "selected Supplementary Figure 11 analysis",
        "reviewer_comment": "R2-M4",
        "analysis_role": (
            "Post-generation comparison of the fixed reference-free Figure 2 "
    ...
    for role, path, source in (
        ...
        ("simulated_metadata", SIMULATED_META, "main_figures/Fig2_new"),
        ("simulated_expression", SIMULATED_EXPRESSION, "main_figures/Fig2_new"),
    ):
        manifest.append(
            {
                "role": role,
