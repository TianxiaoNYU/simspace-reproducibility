...
Status: production scripts complete; the 18-dataset experiment has not been
run. Figure placement and numbering remain undecided.
...
| stLearn | `spatial-domain-benchmark-stlearn` | no image; physical-distance plus expression SME normalization, PCA, then k-means with `K=4` |
...
path, select only physical-distance plus expression weights, and assert in the
pilot that changing the placeholder leaves the chosen result byte-identical.
...
A compact main-figure candidate would contain:

- **A:** reference-free generation and truth-isolation schematic;
- **B:** preselected moderate curved-layer and MRF truth maps with the six
  predictions;
- **C:** ARI across easy, moderate, and hard signal levels, faceted by pattern;
- **D:** boundary F1 plus completion rate, faceted by pattern.
