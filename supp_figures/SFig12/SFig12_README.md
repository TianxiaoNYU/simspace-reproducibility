...
This folder contains everything needed to reproduce the code-generated panels in **Supplementary Figure 12**, including panel-specific data and (when needed) panel-specific external code.
...
- `SFig12.py`  
  Entry script to reproduce Supplementary Figure 12 (generates the figure and/or panel outputs).
...
### Default (SimSpace)
Most of Supplementary Figure 12 should run in the default environment:
```bash
...
R script to draw the heatmap in panel B of Supplementary Figure 12, using the cache `Panel_B_C_D_data/fitted_pvals.csv`. R packages `ggplot2` and `pheatmap` are necessary.
